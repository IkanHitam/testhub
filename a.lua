...
function Speed_Library:CreateWindow(Config)
  ...
  function Tabs:CreateTab(Config)
    ...
    local Title = Config.Title or "Live Table"
    ...
    Custom:Create("UICorner",{CornerRadius = UDim.new(0,4)},Holder)

    local Header = Custom:Create("TextLabel",{
        BackgroundTransparency = 1,
    ...
    local Rows = Custom:Create("ScrollingFrame",{
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
    ...
    local Layout = Custom:Create("UIListLayout",{
    ...
    for i,v in ipairs(Columns) do
        ...
        Custom:Create("TextLabel",{
            BackgroundTransparency = 1,
            Position = UDim2.new((i-1)*ColumnWidth,0,0,0),
    ...
    Custom:Create("TextLabel",{
        BackgroundTransparency = 1,
        Position = UDim2.new(#Columns * ColumnWidth,0,0,0),
        Size = UDim2.new(ColumnWidth,0,1,0),
        Font = Enum.Font.GothamBold,
        Text = "Action",
        TextColor3 = Color3.new(1,1,1),
    ...
    function TableFuncs:SetData(Data)

        TableData = Data

        for _,v in pairs(Rows:GetChildren()) do
            if v:IsA("Frame") then
                v:Destroy()
            end
        end
        ...
        for i,row in ipairs(TableData) do
            ...
            Custom:Create("UICorner",{CornerRadius = UDim.new(0,3)},Row)

            for colIndex,colName in ipairs(Columns) do

                Custom:Create("TextLabel",{
                    BackgroundTransparency = 1,
                    Position = UDim2.new((colIndex-1)*ColumnWidth,0,0,0),
                    Size = UDim2.new(ColumnWidth,0,1,0),
                    Font = Enum.Font.Gotham,
                    Text = tostring(row[colName] or ""),
                    TextColor3 = Color3.new(1,1,1),
                    TextSize = 11
                },Row)
            ...
            local ActionFrame = Custom:Create("Frame",{
                BackgroundTransparency = 1,
                Position = UDim2.new(#Columns * ColumnWidth,0,0,0),
                Size = UDim2.new(ColumnWidth,0,1,0)
            },Row)
            ...
            for _,action in pairs(Actions) do
                ...
                local Btn = Custom:Create("TextButton",{
                    ...
                    Position = UDim2.new(0.5,-11,0.5,-9),
                    Font = Enum.Font.GothamBold,
                    Text = action.Icon or "?",
                    TextColor3 = Color3.new(1,1,1),
                    TextSize = 11
