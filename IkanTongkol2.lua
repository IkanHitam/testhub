...
function IkanTongkol:_rebuild()
    -- Putus koneksi slider lama agar tidak bocor (memory leak fix)
    if self._rebuildConns then
        for _, conn in ipairs(self._rebuildConns) do conn:Disconnect() end
    end
    self._rebuildConns = {}

    -- Hapus semua anak scroll (termasuk UIListLayout & UIPadding agar tidak menumpuk)
    ...
    for i, ctrl in ipairs(self._activeTab.controls) do
        ...
        elseif ctrl.kind == "slider" then
            ...
            -- Simpan koneksi agar bisa diputus saat rebuild berikutnya (cegah memory leak)
            table.insert(self._rebuildConns, UserInputService.InputChanged:Connect(function(inp)
                if slideDrag and (inp.UserInputType == Enum.UserInputType.MouseMovement
                or inp.UserInputType == Enum.UserInputType.Touch) then
                    applySlider(inp.Position.X)
                end
            end))
            table.insert(self._rebuildConns, UserInputService.InputEnded:Connect(function(inp)
                if inp.UserInputType == Enum.UserInputType.MouseButton1
                or inp.UserInputType == Enum.UserInputType.Touch then
                    slideDrag = false
                end
...
function IkanTongkol:Destroy()
    -- Putus semua koneksi slider sebelum destroy
    if self._rebuildConns then
        for _, conn in ipairs(self._rebuildConns) do conn:Disconnect() end
    end
